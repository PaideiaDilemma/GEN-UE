...
\label{fig:circuit_b}
\end{figure}
Note, that the component $R_6$ does not influence our citrcuit at all. To simplify things, we substitute the timevalue $t$ with  $(t-T_0) = t - T_0$. 

\subsubsection*{Derive DGL with a mesh equation (KVL)}
...
.\] 
dividing by L and define $R_4+R_5$ as $R_{45}$, we get
\[
...
characteristic polynom of our DGL, which is $-\lambda + \frac{R_{45}}{L} \overset{!}{=} 0$.
$\lambda$ is equal to  $\frac{R_{45}}{L}$ and the time constant $\tau$ is equal to the inverse of
...
.\] 
This yields us the general Solution of our differential equation.
\[
